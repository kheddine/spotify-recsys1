...
class SpotifyRecSysConstrained {
  ...
  standardScale(matrix) {
    ...
    // Scale - use loops instead of map to avoid stack issues
    const scaled = [];
    for (let i = 0; i < matrix.length; i++) {
      const row = [];
      for (let j = 0; j < numFeatures; j++) {
  ...
  recommendByMoodVector(moodVector, nRecommendations = 10, excludeIndices = []) {
    ...
    for (let idx = 0; idx < this.featureMatrixScaled.length; idx++) {
      if (excludeIndices.includes(idx)) {
        similarities.push(-1);
      } else {
        similarities.push(this.cosineSimilarity(moodVector, this.featureMatrixScaled[idx]));
      }
    }
    ...
    for (let i = 0; i < similarities.length; i++) {
      indexed.push({ sim: similarities[i], idx: i });
    }
    ...
    const result = [];
    for (let i = 0; i < indexed.length && i < nRecommendations; i++) {
      if (indexed[i].sim > 0) {
        result.push([indexed[i].idx, indexed[i].sim]);
      }
    }
