...
class SpotifyRecSysConstrained {
  constructor(df) {
    this.df = JSON.parse(JSON.stringify(df)); // Deep copy
    this.featureCols = [
  ...
  normalizeFeatures() {
    ...
    if (this.df[0]['loudness'] !== undefined) {
      const loudness = this.df.map(row => row['loudness']);
      const minLoudness = Math.min(...loudness);
      const maxLoudness = Math.max(...loudness);
      const range = maxLoudness - minLoudness || 1;

      this.df.forEach(row => {
        row['loudness_norm'] = (row['loudness'] - minLoudness) / range;
      });
    }

    if (this.df[0]['tempo'] !== undefined) {
      const tempo = this.df.map(row => row['tempo']);
      const minTempo = Math.min(...tempo);
      const maxTempo = Math.max(...tempo);
      const range = maxTempo - minTempo || 1;

      this.df.forEach(row => {
        row['tempo_norm'] = (row['tempo'] - minTempo) / range;
      });
    }
  }
  ...
  buildFeatureMatrix() {
    ...
    for (const col of this.featureCols) {
      if (this.df[0][col] !== undefined) {
        features.push(this.df.map(row => row[col]));
      } else if (col === 'loudness' && this.df[0]['loudness_norm'] !== undefined) {
        features.push(this.df.map(row => row['loudness_norm']));
      } else if (col === 'tempo' && this.df[0]['tempo_norm'] !== undefined) {
        features.push(this.df.map(row => row['tempo_norm']));
      }
    }

    // Transpose: convert feature arrays into row vectors
    if (features.length === 0) return [];
    return this.df.map((_, i) => features.map(col => col[i]));
  }
