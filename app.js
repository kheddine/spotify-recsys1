...
async function initializeSystem() {
  ...
  try {
    ...
    // Check file size (limit to 50MB to avoid issues)
    if (file.size > 50 * 1024 * 1024) {
      alert('File too large (max 50MB). Try with a smaller dataset.');
      return;
    }
    ...
    if (!df || df.length === 0) {
      alert('Failed to parse CSV. Check column names:\ntrack_name, artist_name, genre, acousticness, danceability, energy, instrumentalness, liveness, loudness, speechiness, tempo, valence, popularity');
      return;
    }

    console.log(`Parsed ${df.length} tracks`);
